...
{:else if node.nodeType === 'embedded-entry-block'}
  ...
  <div>
    <A to={`${node.data.target.sys.contentType.sys.id.replace('shelf', 'shelve')}s/${node.data.target.fields.identifier}`}><span class='huge'>{node.data.target.fields.title}</span></A>
  </div>
